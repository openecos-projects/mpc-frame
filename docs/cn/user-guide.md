...
尚未获取发行包时，先阅读 [User Kit 获取与使用](user-kit.md)。
...
需要 Python 3、GNU Make、C++ 编译器和 Verilator 5.050。
...
默认顶层模块名由设计名称生成。确实需要自定义时使用 `TOP_NAME`：
...
例如 `io_in[0]` 对应 `user_io[7]`。测试环境只能驱动设计释放的输入引脚。
Frame 构建会自动注入 contention monitor；如果测试环境和设计的输出使能在任一
payload 位重叠，仿真会直接报错，即使双方驱动相同值。不要重命名模板基础设施中的
`reset`、`test_io_oe` 和 `dut`，monitor 通过这些名称连接检查信号。
...
`trace` 生成 FST，`wave` 自动定位并用 GTKWave 打开它。指定了非默认 Frame
测试时，两个目标应传入相同的 `TEST=<name>`。
...
维护者合并时在开发仓库中分配 `DESIGN_ID` 并执行正式集成。该流程分配最终槽位、
更新正式 registry、重新生成 registry RTL，并使用最终 ID
再次运行 Frame 测试。
...
- `payload IO contention`：TB 的 `test_io_oe` 驱动了设计已经使能输出的 payload 位；
  根据打印的 mask 释放对应外部驱动。
- `$fatal`：TB 检查失败，错误文字会说明具体位置。
- `generated file is stale`：这是维护者正式集成阶段的问题，由维护者重新生成 registry。
