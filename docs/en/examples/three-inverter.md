...
[中文说明](../../cn/examples/three-inverter.md)
...
```sh
make create NAME=three-inverter
```
...
output pins at `user_io[9:7]`; the generated monitor terminates the simulation
if the external and design output enables overlap.
...
- Output is `z`: the matching `io_oe` bit is not 1, or design 1 is not registered
  and selected.
- `payload IO contention`: the test drives a design output pin; clear the
