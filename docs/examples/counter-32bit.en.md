...
those pins through `io_oe[31:0]`. Driving from both sides produces `x` values
or IO contention.
...
- Output is `x`: the testbench also drives `user_io[38:7]` and conflicts with the
  design.
