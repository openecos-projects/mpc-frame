...
| 5. 用户设计注册 | 已完成：用户按名称开发，维护者合并时分配 1～127 的最终槽位 | 每个 `designs/<name>/design.json` 自包含且不要求 ID；Frame 测试自动分配临时 ID；根 registry 保存最终 ID 与 package 路径 | `designs/`、注册生成器、`rtl/generated/FrameDesignRegistry.sv`、[用户设计注册](user-design-registration.md) | `make stage5-test` 覆盖独立测试、临时 Frame 集成和正式注册 |
