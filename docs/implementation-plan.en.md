...
contract is documented in [Design Control](design-control.en.md).
...
| 1. Project boundaries | Stabilize source and reference boundaries | Commit `reference/sim`; remove nested Git/build output | Fixed reference source and root Makefile | A clone contains the complete reference |
...
| 3. Selection and clocks | Stable, glitch-free runtime control | Synchronizer, clock gate, stopped/reset unselected designs | control RTL and [design control](design-control.en.md) | `make control-test` covers locking, reselection, reset, isolation, and pulse width |
...
| 5. User registration | Isolated tests and manifest integration | package manifests, generated wrappers/registry/filelist | generator, generated registry, [registration contract](user-design-registration.en.md) | `make stage5-test` covers standalone and Frame paths |
| 6. Frame simulation | One root simulation entry | SV package tests plus reference C++ harness, logs and FST | runner, tests JSON, [regression guide](simulation-regression.en.md) | design 0 and user designs run through `make frame-test` |
| 7. Design regression | Verify mux and multi-design behavior | temporary smoke design, control and contention monitors | layered logs and fast/full runners | fast regression covers root and registered designs |
...
| 9. User template | Deliver a runnable package workflow | generator, RTL/TB templates, bilingual guides | `designs/template`, `make create-design`, stage 9 smoke test | a fresh package passes lint, unit, and temporary Frame integration |
...
2. Maintain the `reference/sim` source boundary and version information.
